      ...
      <div className="flex h-16 items-center justify-between px-4 md:px-6">
        ...
        <div className="flex items-center gap-3">
          ...
          <div className="hidden md:block">
            <h1 className="text-lg font-bold text-nova-text">
              {t(`nav.${pathname.replace('/dashboard', '').replace('/appointments', '').replace('/patients', '').replace('/dentists', '').replace('/services', '').replace('/gallery', '').replace('/settings', '').replace('/', '') || 'dashboard'}`)}
            </h1>
          </div>
        </div>
        ...
        <div className="hidden max-w-md flex-1 md:block">
          <div className="relative">
            <svg
              className="absolute left-3 top-1/2 -translate-y-1/2 text-nova-text-muted"
              width="16"
              height="16"
              viewBox="0 0 24 24"
              fill="none"
            ...
            <input
              ...
              className="w-full rounded-lg border border-nova-border bg-nova-muted py-2 pl-10 pr-4 text-sm text-nova-text placeholder:text-nova-text-muted transition-colors focus:border-nova-primary focus:bg-nova-surface focus:outline-none focus:ring-2 focus:ring-nova-primary/20"
            />
          </div>
        </div>
        ...
        <div className="flex items-center gap-2">
          ...
          {/* Notifications */}
          <button className="relative flex h-9 w-9 items-center justify-center rounded-lg border border-nova-border bg-nova-surface text-nova-text transition-colors hover:bg-nova-muted">
            <svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
          ...
          <div className="flex items-center gap-2">
            <div className="flex h-9 w-9 items-center justify-center rounded-full bg-nova-primary text-white text-sm font-bold">
              {currentDentist ? currentDentist.firstName.charAt(0) : user?.user_metadata?.first_name?.charAt(0) || user?.email?.charAt(0) || '?'}
            </div>
